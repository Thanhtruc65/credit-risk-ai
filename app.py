...
# --- DATABASE CONFIG ---
# Đổi thành True sau khi bạn chạy file `setup_sql.py` thành công
USE_SQL_DB = True 
DB_CONN_STR = 'Driver={ODBC Driver 17 for SQL Server};Server=LAPTOP-STR3U83B\\SQLEXPRESS01;Database=DoAn;Trusted_Connection=yes;'
...
def read_docs(request: Request):
    return templates.TemplateResponse("docs.html", {"request": request, "active_page": "docs"})
...
def get_dashboard_data():
    if not USE_SQL_DB:
        raise HTTPException(status_code=500, detail="Dashboard requires SQL Server connection")
    ...
    try:
        ...
        total_clients = pd.read_sql("SELECT COUNT(*) as Total FROM Clients", conn).iloc[0]['Total']
        high_risk_clients = pd.read_sql("SELECT COUNT(*) as Total FROM Clients WHERE TARGET=1", conn).iloc[0]['Total']
...
def lookup_client(sk_id: int):
    if USE_SQL_DB:
        try:
            import pyodbc
            conn = pyodbc.connect(DB_CONN_STR)
            client_df = pd.read_sql(f"SELECT * FROM Clients WHERE SK_ID_CURR = {sk_id}", conn)
            conn.close()
            if not client_df.empty:
                ...
                return data
            raise HTTPException(status_code=404, detail="Client ID không tồn tại trong SQL.")
        except Exception as e:
    ...
    if request.SK_ID_CURR > 0:
        if USE_SQL_DB:
            try:
                import pyodbc
                conn = pyodbc.connect(DB_CONN_STR)
                client_hist = pd.read_sql(f"SELECT * FROM Clients WHERE SK_ID_CURR = {request.SK_ID_CURR}", conn)
                conn.close()
...
async def chat(request: ChatRequest):
    try:
        ...
        prompt = f"""
        Bạn là một chuyên gia tư vấn tài chính thông minh cho hệ thống AI Loan Predictor.
        Đơn vị tiền tệ mặc định là Việt Nam Đồng (VND/₫).
        Dưới đây là thông tin của người dùng vừa nhập vào form:
        {context_str}
        ...
        Lưu ý: Bạn có thể sử dụng định dạng Markdown cơ bản như **chữ đậm**, *chữ nghiêng*, 
        hoặc ### danh sách để trình bày rõ ràng.
        """
